...
export function DriverRouteSummary({ route }: DriverRouteSummaryProps) {
  const metrics = [
    ...
    { label: 'Peso kg', value: route.total_weight_kg ?? '0' },
    { label: 'Volumen m³', value: route.total_volume_m3 ?? '0' },
  ]
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-bold text-slate-950">{route.route_code}</h2>
