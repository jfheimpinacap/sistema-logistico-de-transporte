...
export function GeoPointDetails({ point }: { point?: GeoMapPoint | null }) {
  if (!point) return <div className="rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-500">Selecciona un punto del mapa para ver su detalle técnico.</div>
  const metadata = point.metadata ?? {}
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-600">Punto seleccionado</p>
      <h3 className="mt-2 text-lg font-bold text-slate-950">{point.label}</h3>
      <dl className="mt-4 space-y-2 text-sm">
        ...
        {Object.entries(metadata).map(([key, value]) => value === null || value === undefined || value === '' ? null : (
          <div key={key}><dt className="text-slate-500">{key}</dt><dd className="mt-1 text-slate-900">{String(value)}</dd></div>
        ))}
      </dl>
    </div>
  )
