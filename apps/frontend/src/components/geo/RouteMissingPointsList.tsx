...
export function RouteMissingPointsList({ points }: { points: GeoMapPoint[] }) {
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-amber-200 bg-white p-5 shadow-sm">
      <h3 className="text-base font-bold text-slate-950">Paradas sin coordenadas</h3>
      <p className="mt-1 text-sm text-amber-700">Completa latitud/longitud en Maestros &gt; Direcciones. Los valores vacíos, incompletos o fuera de rango no se dibujan.</p>
      <div className="mt-4 max-h-[36rem] space-y-3 overflow-y-auto pr-1">
        {points.map((point) => (
          <div key={String(point.id)} className="rounded-xl bg-amber-50 p-3 text-sm">
            <p className="break-words font-semibold text-slate-900">#{point.sequence ?? '—'} · {point.label}</p>
