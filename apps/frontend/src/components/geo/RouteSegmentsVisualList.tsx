...
export function RouteSegmentsVisualList({ segments, points }: { segments: GeoMapSegment[]; points: GeoMapPoint[] }) {
  ...
  if (segments.length === 0) return <div className="rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-500">No hay segmentos lineales disponibles para esta ruta.</div>
  return (
          ...
          return (
            <div key={`${segment.fromPointId}-${segment.toPointId}-${index}`} className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm">
              ...
              <div className="mt-3 grid gap-2 text-xs text-slate-600 md:grid-cols-2">
                <p><span className="font-semibold text-slate-800">Origen:</span> {from?.label ?? '—'}<br /><span className="font-mono">{normalizeCoordinateLabel(from?.latitude, from?.longitude)}</span></p>
                <p><span className="font-semibold text-slate-800">Destino:</span> {to?.label ?? '—'}<br /><span className="font-mono">{normalizeCoordinateLabel(to?.latitude, to?.longitude)}</span></p>
              </div>
